...
public class Promise<T>{
	 ...
	 */
	//Must be synchronized because the subscribe function may
	//change the list while we make the callback
	public synchronized void resolve(T result){
		this.result = result;
		resolved = true;
		for(callback callback0 :callbackList) {
			callback0.call();
		}
	}
	...
	//Must be synchronized because the result function may access the
	//list while this function tries to insert a new callback
